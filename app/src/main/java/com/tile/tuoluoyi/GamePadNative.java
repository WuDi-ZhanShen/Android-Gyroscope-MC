...
public class GamePadNative {
    static boolean isUInputCreated = false, isUHidCreated = false, isInputManagerCreated = false, isBroadcastSent = false;
...
//            //用户使用nohup命令启动，scanner捕捉不到任何输入,会抛出异常。依然需要使用socket占据一个端口，使得程序不退出。另外的好处就是避免重复启动
//            try {
...
//        }
    ...
    private static boolean getInputManager() {
        try {
            ...
            if (im == null) {
                System.err.println("Unable to get inputManager for mode3.");
                return false;
            }
        ...
        try {
            ...
            IBinder binder = new IGamePad.Stub() {
                ...
                public void syncPrefs(boolean invX, boolean invY, int sensityX, int sensityY) throws RemoteException {
                    ...
                    sensitivityXMode1 = ((invertX ? -1 : 1) << 8) * sensityX / 100f;
                    sensitivityYMode1 = ((invertY ? -1 : 1) << 8) * sensityY / 100f;
            ...
            intent = new Intent("intent.tuoluoyi.sendBinder");
